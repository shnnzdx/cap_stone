...
**后端里不依赖 AI 的部分全部做完了,153 条测试守着。前端已接通,Trip workspace 能嵌进主站。剩下的是把真实 AI 质量补上。**

| | |
|---|---|
| 代码 | `backend/`,约 6800 行(含测试) |
| 接口 | **38 个**,http://localhost:8000/docs 可直接点着试 |
| 测试 | 后端 **153 passed**;前端 `npm test` **7 passed**;`trip npm run build` 通过 |
...
```bash
cd backend
DISABLE_SCHEDULER=1 MOCK_AI=1 .venv/bin/python -m pytest -q     # 153 passed
