...
| 接口 | **40 个**,http://localhost:8000/docs 可直接点着试 |
...
| 创建旅行 · My Trips · 我是谁 | ✅ |
| 邀请 token · 访客/账户加入 · 撤销 | ✅ |
| 注册 · 密码登录 · Bearer session · 登出 · 空账户创建第一趟旅行 | ✅ |
...
| API | `backend/app/api/main.py` 扩到 40 个 route | 账户级接口不再错误依赖已有 membership；trip 内接口仍由 `current_membership()` 解析角色 |
...
| 4 | 账户体系补完 | 注册、邮箱+密码登录和 bearer session 已完成；magic link、Guest 绑定账户还没做 |
| 5 | Trip 四状态按日期自动流转 | 无 |
