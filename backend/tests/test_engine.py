...
def test_solo_trip_does_not_send_a_touched_item_to_a_group_round():
    """一个人的行程里,"有人已经对这个时段有别的想法"的那个人只能是自己。"""
    before = item(settledness=Settledness.TOUCHED)
    verdict = classify(
        change(before, item(start_hour=15.0), member_count=1),
        [],
    )

    assert verdict.path is Path.NOTICE


def test_solo_trip_does_not_reopen_a_round_for_a_settled_item():
    before = item(settledness=Settledness.SETTLED)
    verdict = classify(
        change(before, item(start_hour=15.0), member_count=1),
        [],
...
def test_changing_an_item_you_touched_yourself_is_not_contested():
    before = item(settledness=Settledness.TOUCHED)
    verdict = classify(
        change(
            before,
            item(start_hour=15.0),
            member_count=4,
            requested_by_membership_id="m1",
            last_touched_by_membership_id="m1",
        ),
        [],
    )
...
def test_changing_an_item_someone_else_touched_still_needs_a_round():
    before = item(settledness=Settledness.TOUCHED)
    verdict = classify(
        change(
            before,
            ...
            member_count=4,
            requested_by_membership_id="m1",
            last_touched_by_membership_id="m2",
        ),
        [],
    )

    assert verdict.path is Path.ROUND


def test_unknown_member_count_keeps_the_strict_behaviour():
    """调用方没查成员数时必须按"有别人"处理,不能默默放宽。"""
    before = item(settledness=Settledness.TOUCHED)
    verdict = classify(change(before, item(start_hour=15.0)), [])

    assert verdict.path is Path.ROUND


def test_a_solo_trip_still_protects_a_confirmed_booking():
    before = item(settledness=Settledness.BOOKED)
    verdict = classify(
        change(before, item(start_hour=15.0), member_count=1),
        [],
    ...
    assert verdict.path is Path.CONFIRM
