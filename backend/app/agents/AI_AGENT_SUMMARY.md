...
### 22.8 The vote card never says why the vote exists

The assistant already writes a precise conflict analysis in chat and passes it to
`propose_options`. A real one from a trace run:
...
**That sentence is then thrown away.** The vote card shows a fixed line that is
identical on every round — "Vote on the option, not the person" — so a member who
was not in the chat sees that the block is contested but never learns why.

The fix is the same pattern as the options in section 19: **the assistant writes it
once, it is validated and frozen, and the vote only displays it.** No new model
call, no new table.

- `DecisionRound.reason` already exists — reopen rounds use it. It is empty for
  normal rounds.
- Frontend: send `conflict_description` alongside `options` on submit.
- Backend: store it in `reason`, truncated.
- Frontend: the card renders `reason` only when `isReopen`; drop that condition.

**Constraint:** this text is written by the model and will be read by the whole
group while they vote. Treat it exactly like the option copy — plain text,
truncated, and **never an input to any decision.** Classification stays with the
Python rules.

### 22.9 City names match exactly
