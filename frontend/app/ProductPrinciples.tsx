      ...
      <div className="principle-stage">
        {principles.map((principle, index) => {
          ...
          return <div className={`principle-line ${principle.tone}`} key={principle.word}><div className="principle-mask">
            <div className="principle-word" style={{ opacity: .52 + reveal * .48, transform: `translateY(${(1 - reveal) * 112}%) scale(${.985 + reveal * .015})` }}><span style={{ transform: `scale(${.84 + reveal * .16})` }}>{principle.symbol}</span><strong>{principle.word}</strong><small>{principle.number}</small></div>
          </div></div>;
        })}
      </div>
    </div>
