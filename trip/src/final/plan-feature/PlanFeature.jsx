...
function LoadedPlanFeature({ currentTrip, onCommand }) {
  ...
  return <div className={cx('planSplit', !view.drawerItem && 'withMap', view.drawerItem && 'withAssistant')}>
    <section className="planMainPane">
      <div className="pageHeading planHeading"><div className="planSummaryIntro"><span className="eyebrow">Current Plan</span><h1>Your shared itinerary</h1><p>Plan the days, places, and decisions your group will share.</p><div className="planSummaryStats"><span><b>{view.days.length}</b> days</span><span><b>{view.days.reduce((total, day) => total + day.items.length, 0)}</b> stops</span><span><b>{view.days.reduce((total, day) => total + day.items.filter(item => !item.isMeal).length, 0)}</b> activities</span><span><b>{view.days.reduce((total, day) => total + day.items.filter(item => item.isMeal).length, 0)}</b> meals</span></div></div><div className="planCollaboratorStrip"><span>Collaborators</span><div className="collaboratorAvatars"><i>{app.currentUser?.initials || 'You'}</i>{(currentTrip.people || 1) > 1 && <i className="collaboratorCount">+{(currentTrip.people || 1) - 1}</i>}</div><Link to={tripHref(currentTrip.id, 'members')}>Manage members →</Link></div></div>
